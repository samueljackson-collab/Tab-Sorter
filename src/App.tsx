...
export default function App() {
  ...
  const [folderSortBy, setFolderSortBy] = useState<
    'manual' | 'name' | 'groupCount' | 'createdAt' | 'updatedAt'
  >('manual');
  ...
  const handleGenerateTags = async (groupId: string) => {
    ...
    const generateTagsForGroup = (group: TabGroupType) => {
      const allText = group.tabs
        .map((t) => `${t.title} ${t.url}`)
        .join(' ')
        .toLowerCase();
      const tags = [];
      if (allText.includes('github') || allText.includes('code') || allText.includes('react'))
        tags.push('development');
      if (allText.includes('design') || allText.includes('figma') || allText.includes('color'))
        tags.push('design');
      if (allText.includes('news') || allText.includes('article') || allText.includes('blog'))
        tags.push('reading');
      if (allText.includes('youtube') || allText.includes('video') || allText.includes('music'))
        tags.push('media');
  ...
  const handleToggleKeepOpen = (id: string) => {
    setTabs((prevTabs) => prevTabs.map((t) => (t.id === id ? { ...t, keepOpen: !t.keepOpen } : t)));
    setGroups((prevGroups) =>
      prevGroups.map((g) => ({
        ...g,
        tabs: g.tabs.map((t) => (t.id === id ? { ...t, keepOpen: !t.keepOpen } : t)),
      }))
    );
  };
  ...
  const handleCloseTab = (tabId: string) => {
    ...
    setGroups((prev) =>
      prev
        .map((g) => ({ ...g, tabs: g.tabs.filter((t) => t.id !== tabId) }))
  ...
  const handleBatchClose = () => {
    ...
    setGroups((prev) =>
      prev
        .map((g) => ({ ...g, tabs: g.tabs.filter((t) => !idsToClose.includes(t.id)) }))
        .filter((g) => g.tabs.length > 0)
    );
  ...
  const handleDragEnd = (event: any) => {
    ...
    if (over && active.id !== over.id) {
      const activeIsGroup =
        groups.some((g) => g.id === active.id) ||
        folders.some((f) => f.groups.some((g) => g.id === active.id));
      ...
      if (activeIsFolder && overIsFolder) {
        ...
        setFolders((prev) =>
          prev.map((f) => ({ ...f, groups: f.groups.filter((g) => g.id !== active.id) }))
        );
        ...
        setFolders((prev) =>
          prev.map((f) => (f.id === over.id ? { ...f, groups: [...f.groups, groupToMove!] } : f))
        );
  ...
  const handleUpdateFolderAppearance = (
    folderId: string,
    appearance: { color?: string; icon?: string; category?: string }
  ) => {
    setFolders((prev) =>
      prev.map((f) => (f.id === folderId ? { ...f, ...appearance, updatedAt: Date.now() } : f))
    );
  };
  ...
  const handleBatchGroupTabs = (folderId?: string) => {
    ...
    if (folderId) {
      setFolders((prev) =>
        prev.map((f) => (f.id === folderId ? { ...f, groups: [...f.groups, newGroup] } : f))
      );
    } else {
      ...
      <div className="w-[400px] h-[600px] bg-slate-50 dark:bg-slate-900 shadow-2xl rounded-lg flex flex-col font-sans overflow-hidden relative border border-slate-200 dark:border-slate-800">
        ...
        <header className="p-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center flex-shrink-0 bg-white dark:bg-slate-900 z-10">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => setIsAISorterOpen(true)}
              className="p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 text-indigo-600 dark:text-indigo-400"
              title="AI Sorter"
            >
              <Sparkles size={18} />
            </button>
        ...
        <DndContext onDragEnd={handleDragEnd}>
          <main className="flex-grow p-4 overflow-y-auto space-y-6 bg-slate-50 dark:bg-slate-900">
            <div className="p-4 border-b border-slate-200 dark:border-slate-800">
              <h2 className="text-sm font-semibold text-slate-500 dark:text-slate-400 mb-2 px-2">
                Create Folder
              </h2>
              ...
              <div className="flex items-center gap-2 mb-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    <SortAsc
                      size={16}
                      className={`transition-transform ${folderSortDirection === 'desc' ? 'rotate-180' : ''}`}
                    />
                  </button>
            ...
            <div className="space-y-6">
              {(() => {
                const categories = Array.from(
                  new Set(sortedFolders.map((f) => f.category || 'Uncategorized'))
                );
                    ...
                    {categories.map((category) => {
                      ...
                      return (
                        <div key={category} className="space-y-2">
                          {categories.length > 1 && (
                            <h3 className="text-xs font-bold uppercase text-slate-500 dark:text-slate-400 px-2">
                              {category}
                            </h3>
                          )}
                          <div
                            className={
                              folderView === 'grid' ? 'grid grid-cols-2 gap-4' : 'space-y-4'
                            }
                          >
                            {categoryFolders.map((folder) => {
                              const folderTabIds = new Set(
                                folder.groups.flatMap((g) => g.tabs.map((t) => t.id))
                              );
                              const selectedTabsInFolder = Array.from(selectedTabIds).filter((id) =>
                                folderTabIds.has(id)
                              );
                              ...
                              return (
                                <FolderItem
                                  ...
                                  onBatchRenameTabs={(newName) =>
                                    handleBatchRenameTabs(folder.id, newName)
                                  }
              ...
              <div className="flex items-center gap-2 flex-grow">
                ...
                {isBatchRenaming ? (
                  <div className="flex items-center gap-2 flex-grow">
                    ...
                    />
                    <button
                      onClick={handleBatchRenameFolders}
                      className="p-1 bg-indigo-600 text-white rounded hover:bg-indigo-700"
                    >
                      <Check size={14} />
                    </button>
                    <button
                      onClick={() => setIsBatchRenaming(false)}
                      className="p-1 bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded hover:bg-slate-300 dark:hover:bg-slate-600"
                    >
                      <X size={14} />
                    </button>
                  </div>
            ...
            >
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="p-2 rounded-md hover:bg-slate-700 text-xs font-semibold"
                >
                  Clear
                </button>
              </div>
