    ...
    >
      {viewMode === 'grid' ? (
        <div className="flex-grow flex flex-col p-3 relative h-full">
          <div className="flex justify-between items-start mb-2">
            ...
            <div className="flex gap-1">
              <button
                ...
                className="p-1 rounded-full text-slate-500 dark:text-slate-400 hover:bg-black/5 dark:hover:bg-white/5"
              >
                <Settings size={14} />
              </button>
              <button
                onClick={() => onDelete(folder.id)}
                className="p-1 rounded-full text-red-500 hover:bg-red-100/50 dark:hover:bg-red-900/20"
              >
                <Trash2 size={14} />
              </button>
            </div>
          ...
          <div
            className="flex-grow flex flex-col items-center justify-center gap-2 cursor-grab active:cursor-grabbing"
            {...attributes}
            {...listeners}
          >
            ...
            <div className="text-center overflow-hidden w-full px-2">
              <h2 className="font-bold text-slate-800 dark:text-slate-100 truncate text-sm">
                {folder.name}
              </h2>
              <div className="flex items-center justify-center gap-2 mt-1">
                <span className="text-[10px] text-slate-500 dark:text-slate-400">
                  {folder.groups.length} groups
                </span>
                {folder.category && (
          ...
          >
            <ChevronDown
              size={16}
              className={`transition-transform text-slate-600 dark:text-slate-400 ${isOpen ? 'rotate-180' : ''}`}
            />
          </button>
        </div>
      ) : (
        ...
        >
          <div className="flex items-center gap-2 flex-grow overflow-hidden">
            ...
            <div
              className="flex items-center gap-2 flex-grow overflow-hidden cursor-grab active:cursor-grabbing"
              {...attributes}
              {...listeners}
            >
              <SelectedIcon
              ...
              {isEditing ? (
                ...
                <div className="flex items-center gap-2 overflow-hidden">
                  <h2
                    className={`${viewMode === 'compact' ? 'text-sm' : 'text-base'} font-semibold text-slate-800 dark:text-slate-100 hover:text-indigo-600 dark:hover:text-indigo-400 truncate`}
          ...
          <div className="flex items-center gap-1 flex-shrink-0">
            {selectedTabCount > 0 && (
              <div
                className={`flex items-center gap-1 mr-2 bg-white/50 dark:bg-slate-800/50 rounded-md p-0.5 border border-slate-300 dark:border-slate-600 ${viewMode === 'compact' ? 'hidden sm:flex' : ''}`}
              >
                <button
            ...
            <button
              onClick={() => setShowSettings(!showSettings)}
              className="p-1.5 rounded-full text-slate-500 dark:text-slate-400 hover:bg-black/5 dark:hover:bg-white/5"
              title="Settings"
            >
              <Settings size={viewMode === 'compact' ? 14 : 16} />
            </button>
            <button
              onClick={() => onToggleShare(folder.id)}
              className={`p-1.5 rounded-full ${folder.shared ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-400'} hover:bg-black/5 dark:hover:bg-white/5`}
              title="Share"
            >
            ...
            <button
              onClick={() => onDelete(folder.id)}
              className="p-1.5 rounded-full text-red-500 hover:bg-red-100/50 dark:hover:bg-red-900/20"
              title="Delete"
            >
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  >
                    <SortAsc
                      size={16}
                      className={`transition-transform ${tabSortDirection === 'desc' ? 'rotate-180' : ''}`}
                    />
                  </button>
                </div>
              </div>
