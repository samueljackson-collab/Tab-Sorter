...
export function TabItem({
  ...
  const isInactive =
    (new Date().getTime() - tab.lastAccessed.getTime()) / (1000 * 3600 * 24) > inactiveThreshold;
  return (
    <div
    ...
    >
      ...
      <div className="flex-grow truncate">
        <div className="flex items-center gap-2">
          <p
            className={`text-sm truncate ${isSelected ? 'text-indigo-900 dark:text-indigo-100 font-medium' : 'text-slate-800 dark:text-slate-200'}`}
          >
            {tab.title}
          </p>
          {tab.memoryUsage && (
            <span
              className={`text-[10px] flex-shrink-0 px-1.5 py-0.5 rounded-full ${isSelected ? 'bg-indigo-200 dark:bg-indigo-800 text-indigo-700 dark:text-indigo-200' : 'bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-400'}`}
            >
              {tab.memoryUsage} MB
            </span>
          )}
        ...
        <div className="flex items-center gap-2">
          {isInactive && (
            <div
              ...
              title={`Inactive for more than ${inactiveThreshold} days`}
            ></div>
          )}
          <p
            className={`text-xs truncate ${isSelected ? 'text-indigo-500 dark:text-indigo-400' : 'text-slate-400 dark:text-slate-500'}`}
          >
            {tab.url}
          </p>
          <span
            className={`text-xs flex-shrink-0 ${isSelected ? 'text-indigo-500 dark:text-indigo-400' : 'text-slate-400 dark:text-slate-500'}`}
          >
      ...
      <div className="flex items-center gap-1 flex-shrink-0">
        {tab.isDuplicate && (
          <span className="px-2 py-0.5 text-[10px] font-medium bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-300 rounded-full">
            Dup
          </span>
        )}
        {tab.isBroken && (
          <span className="px-2 py-0.5 text-[10px] font-medium bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300 rounded-full">
            Broken
          </span>
        )}
