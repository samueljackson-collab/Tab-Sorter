...
export function AIGroupSuggester({ tab, groups, onSuggestionAccept }: AIGroupSuggesterProps) {
  ...
  if (suggestion) {
    return (
      <div className="flex items-center gap-2 text-xs">
        <span>
          Suggest: <span className="font-semibold text-indigo-600">{suggestion}</span>
        </span>
        <button
  ...
  return (
    <button
      onClick={handleSuggest}
      className="p-1 rounded-full text-slate-400 group-hover:text-indigo-500"
      title="Suggest Group with AI"
    >
      <Wand2 size={14} />
    </button>
  );
