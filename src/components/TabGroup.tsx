...
export function TabGroup({
  ...
  const sortedTabs = React.useMemo(() => {
    ...
    result.sort((a, b) => {
      ...
      else if (tabSortBy === 'lastAccessed')
  ...
  return (
    <div
      ...
      {...attributes}
      {...listeners}
      className="rounded bg-white dark:bg-slate-800 shadow-sm overflow-hidden border border-slate-200 dark:border-slate-700"
    >
        ...
        >
          <div className="flex items-center gap-2">
            <ColorPicker
              ...
              onColorChange={(newColor) => onColorChange(group.id, newColor)}
            />
            ...
            {totalMemory > 0 && (
              <span
                className={`text-[10px] px-1.5 py-0.5 rounded-full ${isColorLight(group.color) ? 'bg-black/10 text-slate-800' : 'bg-white/20 text-white'}`}
              >
                {totalMemory} MB
              </span>
            )}
          ...
          >
            <span
              ...
              style={{
                ...
                color: isColorLight(group.color) ? '#000' : '#fff',
              }}
            >
              {group.tabs.length}
            </span>
        ...
        {group.tags && group.tags.length > 0 && (
          <div className="px-2 py-1 flex flex-wrap gap-1" style={{ backgroundColor: lighterBg }}>
            {group.tags.map((tag, idx) => (
              <span
                key={idx}
                className={`text-[9px] px-1.5 py-0.5 rounded-full border border-black/10 dark:border-white/10 ${textColor} opacity-80`}
              >
                #{tag}
              </span>
            ))}
